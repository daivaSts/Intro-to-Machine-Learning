...
# Reduce the features dimentions
n_components = 8
pca = PCA(n_components=n_components, whiten=True).fit(features_scaled)
...
# plot first pca component to test and visualize
if False:
...
features_train,features_test,labels_train, labels_test = cross_validation.train_test_split(features_scaled, labels, test_size=0.2, random_state=42)
...
if False:
	...
	print "GaussianNB running time: {} s".format(round(time()-t0, 3))
...
if True:
	...
	clf = DecisionTreeClassifier(criterion='gini', min_samples_split=2, max_features=5, random_state=42)
